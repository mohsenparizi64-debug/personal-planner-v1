...
const editingTaskForm = ref({ title: '', priority: 0, due_date: '', is_completed: false })
...
const fetchKPIs = async () => { if (!selectedGoalId.value) return; try { const res = await api.get(`/roadmap/goal/${selectedGoalId.value}/kpis`); kpis.value = res.data } catch (e) {} }
...
const validateForm = () => { validationErrors.value = {}; let hasError = false; if (!taskForm.value.title.trim()) { validationErrors.value.title = 'عنوان تسک الزامی است'; hasError = true } ; if (taskForm.value.duration_days && taskForm.value.duration_days < 0) { validationErrors.value.duration_days = 'مدت زمان نمی‌تواند منفی باشد'; hasError = true } ; if (taskForm.value.recurrence_interval && taskForm.value.recurrence_interval < 1) { validationErrors.value.recurrence_interval = 'دوره تکرار باید حداقل ۱ باشد'; hasError = true } ; return !hasError }
const saveTask = async () => {
  ...
  try {
    ...
    for (const key of ['register_date', 'last_action_date', 'recurrence_end_date']) if (data[key] === '') data[key] = null
...
const saveSubGoal = async () => {
  ...
  try {
    if (editingSubGoal.value) { await api.put(`/roadmap/subgoals/${editingSubGoal.value.id}`, subGoalForm.value); showToast('✅ زیرهدف بروزرسانی شد') }
    else { await api.post(`/roadmap/goal/${selectedGoalId.value}/subgoals`, subGoalForm.value); showToast('✅ زیرهدف جدید ایجاد شد') }
    showSubGoalForm.value = false; resetSubGoalForm(); await fetchSubGoals()
  } catch (e) { showToast('❌ خطا در ذخیره', 'error') }
}

const deleteSubGoal = async (id) => { try { await api.delete(`/roadmap/subgoals/${id}`); showToast('🗑️ زیرهدف حذف شد'); await fetchSubGoals() } catch (e) {} }
const editSubGoal = (sg) => { subGoalForm.value = { title: sg.title, description: sg.description || '', start_date: sg.start_date || '', target_date: sg.target_date || '', status: sg.status, order_index: sg.order_index }; editingSubGoal.value = sg; showSubGoalForm.value = true }
const resetSubGoalForm = () => { subGoalForm.value = { title: '', description: '', start_date: '', target_date: '', status: 'not_started', order_index: 0 }; editingSubGoal.value = null }
const toggleExpand = (id) => { expandedSubGoals.value[id] = !expandedSubGoals.value[id] }
const subGoalProgress = (sg) => { if (!sg.tasks || sg.tasks.length === 0) return 0; return Math.round((sg.tasks.filter(t => t.is_completed).length / sg.tasks.length) * 100) }

const addTask = async (subGoalId) => { const title = newTaskTitle.value[subGoalId]; if (!title?.trim()) return; try { await api.post(`/roadmap/subgoals/${subGoalId}/tasks`, { title, priority: 0 }); newTaskTitle.value[subGoalId] = ''; await fetchSubGoals() } catch (e) {} }
const toggleTask = async (task) => { try { await api.put(`/roadmap/tasks/${task.id}`, { is_completed: !task.is_completed }); await fetchSubGoals() } catch (e) {} }
const deleteTask = async (taskId) => { try { await api.delete(`/roadmap/tasks/${taskId}`); await fetchSubGoals() } catch (e) {} }
const startEditTask = (task) => { editingTaskId.value = task.id; editingTaskForm.value = { title: task.title, priority: task.priority ?? 0, due_date: task.due_date || '', is_completed: !!task.is_completed } }
const cancelEditTask = () => { editingTaskId.value = null; editingTaskForm.value = { title: '', priority: 0, due_date: '', is_completed: false } }
...
const saveKPI = async () => { if (!kpiForm.value.title.trim()) return; try { if (editingKPI.value) { await api.put(`/roadmap/kpis/${editingKPI.value.id}`, kpiForm.value); showToast('✅ KPI بروزرسانی شد') } else { await api.post(`/roadmap/goal/${selectedGoalId.value}/kpis`, kpiForm.value); showToast('✅ KPI جدید ایجاد شد') } showKPIForm.value = false; resetKPIForm(); await fetchKPIs() } catch (e) { showToast('❌ خطا در ذخیره KPI', 'error') } }
...
const kpiPercent = (kpi) => { if (!kpi.target_value) return 0; return Math.min(100, Math.round((kpi.current_value / kpi.target_value) * 100)) }

onMounted(() => { fetchGoals(); fetchCategories() })
</script>

<template>
  <div class="p-6 md:p-10 max-w-6xl mx-auto relative z-10 min-h-screen" :class="themeStore.currentTheme === 'persian-classic' ? 'page-bg-tasks' : themeStore.currentTheme === 'cyber-digital' ? 'page-bg-tasks' : ''">
              ...
              <div class="space-y-1.5 mt-3">
                <div v-for="task in sg.tasks" :key="task.id" class="flex items-center gap-2 text-sm py-1">
                  ...
                  <div v-if="editingTaskId === task.id" class="flex-1 flex flex-wrap gap-2">
                    ...
                    <input v-model="editingTaskForm.due_date" type="date" class="px-2 py-1 rounded-lg text-xs" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }" />
                  ...
                  <div v-else class="flex-1 flex items-center gap-2">
                    ...
                    <span v-if="task.due_date" class="text-[10px] px-2 py-0.5 rounded-full" :style="{ background: 'rgba(255,255,255,0.08)', color: 'var(--text-secondary)' }">📅 {{ task.due_date }}</span>
                  ...
                  <div v-if="editingTaskId !== task.id" class="flex gap-1">
                    ...
                    <button @click="deleteTask(task.id)" class="p-1 rounded hover:bg-red-500/10" :style="{ color: 'var(--text-secondary)' }"><Trash2 class="w-3 h-3" /></button>
                  </div>
                </div>
    ...
    <div v-if="showSubGoalForm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="showSubGoalForm = false">
      <div class="w-full max-w-md rounded-2xl p-6" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2"><input v-model="subGoalForm.start_date" type="date" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }" /><input v-model="subGoalForm.target_date" type="date" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }" /></div>
    ...
    <div v-if="showKPIForm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="showKPIForm = false">
      <div class="w-full max-w-md rounded-2xl p-6" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
        <div class="flex items-center justify-between mb-4"><h3 class="text-lg font-bold" :style="{ color: 'var(--text-primary)' }">{{ editingKPI ? 'ویرایش KPI' : 'KPI جدید' }}</h3><button @click="showKPIForm = false; resetKPIForm()" :style="{ color: 'var(--text-secondary)' }"><X class="w-5 h-5" /></button></div>
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2"><select v-model="kpiForm.unit" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }"><option value="تومان">تومان</option><option value="نفر">نفر</option><option value="ساعت">ساعت</option><option value="عدد">عدد</option><option value="درصد">درصد</option></select><select v-model="kpiForm.frequency" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }"><option value="daily">روزانه</option><option value="weekly">هفتگی</option><option value="monthly">ماهانه</option></select></div>
          <div class="grid grid-cols-2 gap-2"><input v-model.number="kpiForm.target_value" type="number" placeholder="هدف" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }" /><input v-model.number="kpiForm.current_value" type="number" placeholder="فعلی" class="px-3 py-2 rounded-lg" :style="{ background: 'var(--bg-primary)', border: '1px solid var(--border)', color: 'var(--text-primary)' }" /></div>
        </div>
        <div class="flex gap-2 mt-4"><button @click="saveKPI" class="flex-1 py-2 rounded-lg text-white font-semibold" :style="{ background: 'var(--accent)' }">ذخیره</button><button @click="showKPIForm = false; resetKPIForm()" class="px-4 py-2 rounded-lg" :style="{ background: 'var(--bg-hover)', color: 'var(--text-secondary)' }">انصراف</button></div>
      </div>
    </div>
  </div>
</template>